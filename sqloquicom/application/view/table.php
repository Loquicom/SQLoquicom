...
<div id="conf_action" class="hide">
    ...
    <div class="row-fluid" style="padding-top: 1em; padding-bottom: 2em;">
        <div class="offset8 col4">
            ...
            <button id="conf_btn" class="btn btn-primary main-color text-color">Valider</button>
        </div>
    </div>
</div>
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        $('#limit').on('change', function () {
            ...
            if ($(this).val() > 0) {
                location.href = '<?= redirect_url('Affichage/table/' . $nom) ?>/' + Math.round($(this).val());
            }
        });

        //Charge le tableau d'info
        $.post('<?= redirect_url('Affichage/ajx_tableInfo') ?>', {'table': '<?= $nom ?>'}, function (data) {
        ...
        $('.btn_action').on('click', function () {
            ...
            if (action == 'insert') {
                //Ajout de ligne
                var params = prepare_post('#table_content', {'table': '<?= $nom ?>'});
                $.redirectPost('<?= redirect_url('Donnee/insert')?>', params);
            } else if (action == 'update') {
                //Au moins une case est cochée
                if (params.length > 1) {
                    //Mise à jour des lignes concerner
                    var params = prepare_post('#table_content', {'table': '<?= $nom ?>'});
                    $.redirectPost('<?= redirect_url('Donnee/update') ?>', params);
                }
        ...
        $('#dialog').on('click', '.btn_valid_suppr', function () {
            ...
            var params = prepare_post('#table_content', {'table': '<?= $nom ?>'});
            $.post('<?= redirect_url('Donnee/ajx_delete') ?>', params, function (data) {
        ...
        $('#dialog').on('click', '.btn_valid_trunc', function () {
            //Suppression des lignes slectionnées
            var params = prepare_post('#table_content', {'table': '<?= $nom ?>'});
            $.post('<?= redirect_url('Donnee/ajx_truncate') ?>', params, function (data) {
                $('#zone_message').removeClass();
                if (data.etat == 'ok') {
                    $('#zone_message').addClass('alert a-is-success').html(data.message);
                    //Actualisation du contenue de la table
                    updateTab();
                } else if (data.etat == 'err') {
                    $('#zone_message').addClass('alert a-is-danger').html(data.message);
                } else {
    ...
    function updateTab(page = 1, search = '', limit = <?= $limit ?>) {
        ...
        $.post('<?= redirect_url('Affichage/ajx_tableContent') ?>', params, function (data) {
